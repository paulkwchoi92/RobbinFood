...
class StocksTransaction extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (
      ...
      this.state.numOfShares * this.props.currentPrice > this.props.buyingPower
    ) {
      return this.setState({ error: "Not Enough Funds" });
    } else if (
      this.state.formType === "Sell" &&
      this.props.ownedShares < this.state.numOfShares
    ) {
        ...
        <form className="transaction-form">
          ...
          <label>
            ...
            <input
              ...
              min="0"
              onChange={this.handleChange("numOfShares")}
              type="number"
              placeholder="0"
              step="1"
            />
          ...
          <div
            className={`form-div ${this.props.ownedShares ? "bordered" : ""}`}
          >
          ...
          <div
            id="error-container"
            className={this.state.errors > 0 ? "form-div grow" : "form-div"}
          >
            {this.state.error}
          </div>
          <div id="submit-div">
            ...
            >{`Submit ${this.state.formType}`}</button>
          </div>
          <div id="info-div">
            {this.state.formType === "Buy"
              ? `${formatMoney(this.props.buyingPower)} Buying Power Available`
              : `${
                  this.props.ownedShares ? this.props.ownedShares : 0
                } Shares Available`}
          </div>
        </form>
