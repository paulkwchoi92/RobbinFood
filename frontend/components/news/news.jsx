    ...
    return this.state.articles ? (
      <div id="news-container">
        ...
        <div className={this.state.showMore ? "news-items long" : "news-items"}>
          {news.map((article, i) => {
            const publish = moment.tz(
              article.publishedAt.slice(0, article.publishedAt.length - 1),
              "UTC"
            );
            const duration = moment.duration(now.diff(publish));
            return (
              <a key={i} href={article.url} className="news-item" color="black">
                <img
                  src={article.urlToImage}
                  onError={e => {
                    e.target.onerror = null;
                    e.target.src = window.stock_news;
                  }}
                />
