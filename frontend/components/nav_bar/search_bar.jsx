...
import ResultItem from "./search_result";
class SearchBar extends Component {
  ...
  renderSearchResult() {
    ...
    return search.length > 0 ? (
      <div
        className={
          this.state.display && this.state.search !== "" ? " " : "hide"
        }
        id="search-results"
      >
        ...
        <div className="flex">
          <ul>
            {this.state.results.map(result => (
              <ResultItem key={result.symbol} stock={result} />
            ))}
          </ul>
        </div>
      </div>
      ...
      >
        <div
          className={`search-container ${
            this.state.display && this.state.search !== "" ? "long" : ""
          }`}
        >
          <div id="searchbar">
            <svg
